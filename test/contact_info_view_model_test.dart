...
main() {
  ...
  final Subject<DeepLinkResult> onStart = PublishSubject();
  ...
  DeepLinkResult deepLinkResult =
      DeepLinkResult(DeepLinkAction.openContactInfo, value: null);
  ...
  test('test viewmodel init fields correctly', () {
    ...
    viewModel.input.onStart.add(deepLinkResult);
  });
  ...
  test('test viewmodel onLocationButtonPressed', () {
    ...
    debugPrint(updatedList.toString());

    when(mockUserProfileRepo.fetchContactInfo())
        .thenAnswer((_) => Stream.value(contactInfo));
    when(mockLocationRepo.getCurrentLocation())
        .thenAnswer((_) => Stream.value(testCoordinates));
    when(mockLocationRepo.decodeUserLocation(testCoordinates))
        .thenAnswer((_) => Stream.value(testAddress));
  ...
  test('onDeeplinkResult', () {
    ...
    DeepLinkResult deepLinkResult2 =
        DeepLinkResult(DeepLinkAction.openContactInfo, value: fieldsValues);
    ...
    expect(viewModel.output.fieldList,
        emitsInOrder([Result.loading(null), Result.success(list)]));
    viewModel.input.onStart.add(deepLinkResult2);
  });
}
